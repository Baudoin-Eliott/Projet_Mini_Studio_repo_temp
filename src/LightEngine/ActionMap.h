		...
		for (auto combo : it->second)
		{
			...
			for (auto key : combo.inputs)
			{
				if (std::visit([this, &key](auto val)
					{
						if constexpr (std::is_same_v<decltype(val), sf::Keyboard::Key>) {
							switch (key.second)
							{
							...
							case (TriggerState::Released):
								return m_inputManager.isKeyRelease(val);
							default:
								return false;
							}
						}
						else if constexpr (std::is_same_v<decltype(val), sf::Mouse::Button>)
						{
							switch (key.second)
							{
							case (TriggerState::Down):
								return m_inputManager.isMouseButtonDown(val);
							case (TriggerState::Held):
								return m_inputManager.isMouseButtonHeld(val);
							case (TriggerState::Released):
								return m_inputManager.isMouseButtonRelease(val);
							default:
								return false;
							}
						}
